...
module.exports = {
  list: async (req, res) => {
    /*
                ...
                `
            */
    const data = await res.getModelList(Order, {}, ["userId", "pizzaId"]);//! burada array içinde almamın nedeni getModelList'in 3 tane parametre alması gerektiği için array içinde olmasaydı 4 parametre olacaktı ve hata olurdu..
    res.status(200).send({
      error: false,
  ...
  create: async (req, res) => {
    ...
    */
    const pizza = await Pizza.findOne({ _id: req?.body?.pizzaId }); //? order oluştururken pizzaId gelmesi lazım
    if (!pizza) {
